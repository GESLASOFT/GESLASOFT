...
function validarForm() {
  ...
  // Campos requeridos simples (solo "vacío o no")
  [
  ...
  // Teléfono: requerido + exactamente 9 dígitos numéricos
  const telInput = document.getElementById('telefono');
  const telError = document.getElementById('telefono-error');
  const telValor = telInput.value.trim();
  const telRegex = /^\d{9}$/;
  if (!telValor) {
    telInput.classList.add('invalid');
    telError.textContent = 'Este campo es requerido';
    telError.classList.add('visible');
    ok = false;
  } else if (!telRegex.test(telValor)) {
    telInput.classList.add('invalid');
    telError.textContent = 'Ingresa un número válido de 9 dígitos';
    telError.classList.add('visible');
    ok = false;
  } else {
  ...
  // Email: ahora requerido + formato válido
...
['empresa', 'nombre', 'telefono', 'email'].forEach(id => {
...
async function generarNroCotizacion(organizacion_id) {  // ← recibe el parámetro
  const anio = new Date().getFullYear();
  try {
    const data = await sbFetch(
      `cotizaciones?organizacion_id=eq.${organizacion_id}&nro_cotizacion=like.COT-${anio}-*&select=nro_cotizacion&order=creado_en.desc&limit=1`
    );
    let siguiente = 1;
    if (data.length > 0) {
...
async function enviarSolicitud() {
  ...
  try {
    ...
    const usuarioSesion = JSON.parse(sessionStorage.getItem('geslasoft_usuario'));
